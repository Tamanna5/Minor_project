...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='DeepEDM Runner')
    ...
    parser.add_argument('--model', type=str, required=True, default='DeepEDM',
                        help='model name, options: [DeepEDM, Autoformer, Transformer, TimesNet]')
    ...
    parser.add_argument('--model_config', type=str, default='./configs/benchmark_dataset.yaml', help='model config file (YAML)')
    ...
    parser.add_argument('--in_channels', type=int, default=-1, help='override detected input channels when needed')

    args = parser.parse_args()
    args.use_gpu = True if torch.cuda.is_available() else False

    # Resolve model_config path relative to this file when run from outside repo root
    if args.model_config and not os.path.isabs(args.model_config):
        script_dir = os.path.dirname(os.path.abspath(__file__))
        candidate_cfg = os.path.join(script_dir, args.model_config)
        if os.path.exists(candidate_cfg):
            args.model_config = candidate_cfg

    # Force CPU-safe dataloader settings when CUDA is unavailable
    if not args.use_gpu:
        args.num_workers = 0

    # Ensure valid seed even if config is missing
